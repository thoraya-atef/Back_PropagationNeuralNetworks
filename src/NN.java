...
public class NN {
    ...
    // ***************Normalization****************
    public static double CalculateMean(Vector<double[]> v, int idx) {
        double sum = 0.0;
        for (int i = 0; i < K;i++) {
            sum +=v.get(i)[idx];
        }
    ...
    public static double CalculateStandardDiv(Vector<double[]> V, int idx, double Mean) {
        double standardDiv = 0.0;
        for (int j = 0; j < K;j++) {
            standardDiv+= Math.pow(V.get(j)[idx] - Mean, 2);
        }
        standardDiv = Math.sqrt(standardDiv / K);
        return standardDiv;
    }

    public static Vector<double[]> GaussianNormalization(Vector<double[]> V, int M) {
        Vector<double[]> vector = new Vector<>();
        double normalized[] = new double[M];
        double[]  Mean = new double[V.get(0).length];
        double[] std_dev = new double[V.get(0).length];
        for (int i = 0; i < V.get(0).length; i++){
            Mean[i] = CalculateMean(V, i);
            std_dev[i] = CalculateStandardDiv(V, i,Mean[i]);
        }
        for (int j = 0; j < K; j++) {
            for (int i = 0; i < V.get(0).length; i++) {
                normalized[i] = (V.get(j)[i] -  Mean[i]) / std_dev[i];
            }
    ...
    public static double[] ErrorofOutputlayer(double Y_target[], double predicted[]){
        ...
        for (int i = 0; i < N; i++) {
            Error[i] = Y_target[i]-predicted[i];
            Y_error[i] = predicted[i] * (1 - predicted[i]) * Error[i];
        }
        return Y_error;
    }
    public static double[] ErrorofHiddenlayer(double Y_error[],double H[], double Weights_outputLayer[][]){
        double Hidden_error[] = new double[L];
        for (int i = 0; i < L; i++) {
            double Sum = 0;
            for (int j = 0; j < N; j++) {
                Sum += (Y_error[j] * Weights_outputLayer[j][i]);
            }
    ...
    public static double[][] Updated_weights_of_outputlayer(double[][] old_Weights, double learning_rate,
                                                     double[] Hidden_out, double[] Y_error){

        for (int i = 0; i < N; i++) {
            for (int j = 0; j < L; j++) {
                old_Weights[i][j] = old_Weights[i][j] + learning_rate * Hidden_out[j] * Y_error[i];
            }
        }
        return old_Weights;
    }
    public static double[][] Updated_weights_of_Hiddenlayer(double[][] old_Weights, double learning_rate,
                                                     double[] x, double[] Hidden_error){
        for (int i = 0; i < L; i++) {
            for (int j = 0; j < M; j++) {
                old_Weights[i][j] = old_Weights[i][j] + learning_rate * x[j] * Hidden_error[i];

            }
        }
        return old_Weights;
    }
    public static double Sq_error(double Y_targetl[], double Y_Predicted[]){
        double error = 0;
        for (int i = 0; i < N; i++) {
    ...
    public static void FirstProgram() throws IOException {
        ...
        double erors[] = new double[K];
        double MSerors=100000;
        double Min=20000;
        ...
        for (int i=0;i<num_of_iterations;i++){
            for(int j=0;j<K;j++){
                ...
                Errors_Hs=ErrorofHiddenlayer(Errors_Ys,Hidden,WeightsOutput);
                ...
                erors[j] = Sq_error(Y.get(j), Output);
        ...
        System.out.println(" best Mean Square Error  = "+MSerors);
    ...
    public static void SecondProgram() throws IOException {
        ...
        for(int j=0;j<K;j++) {
            ...
            MSerors[j] = Sq_error(Y.get(j), Output);
        }
        double sumerror=0.0;
        for(int j=0;j<K;j++){
            sumerror+=MSerors[j];
